 ...
 * #%L
 * Project: IQM - API
 * File: DynamicResourceLoader.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class DynamicResourceLoader {
		...
		if (dirURL.getProtocol().equals("jar")) {
			...
			while (entries.hasMoreElements()) {
				...
				if (name.startsWith(path)) { // filter according to the path
					...
					if (checkSubdir >= 0) {
						// if it is a subdirectory, just return the directory
						// name
	...
	public static void main(String[] args) {
		try {
			String[] foo =
			// new DynamicResourceLoader().getFileListFromResource("i18n");
			new DynamicResourceLoader().getResourceListing(
					DynamicResourceLoader.class, "samples/image/");
			for (String s : foo) {

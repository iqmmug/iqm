...
public class SampleEntropy {
	 ...
	 * 
	 */
	private Vector<Double> calcCorrelations(
			Vector<Integer> numberOfCorrelations, int m, double d) {

		Vector<Double> correlations = new Vector<Double>();

		for (int n = 0; n < numberOfCorrelations.size(); n++) {
			correlations.add((double) numberOfCorrelations.get(n)
					/ (numbDataPoints - (m - 1) * d - 1)); // -1 because i=j was not allowed
		}
		return correlations;
	}

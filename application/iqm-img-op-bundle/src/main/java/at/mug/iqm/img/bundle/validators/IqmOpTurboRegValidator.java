 ...
 * File: IqmOpTurboRegValidator.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class IqmOpTurboRegValidator extends DefaultImageOperatorValidator {

	public IqmOpTurboRegValidator() {
	}
		...
		if (super.validateSources(wp)) {
			...
			if (!firstValid && !secondValid)
				throw new IllegalArgumentException(
						"This operator requires a single band image at indices 0 and 1 of the source vector!");
			else if (!firstValid)
				throw new IllegalArgumentException(
						"This operator requires a single band image at index 0 of the source vector!");
			else if (!secondValid)
				throw new IllegalArgumentException(
						"This operator requires a single band image at index 1 of the source vector!");
		}

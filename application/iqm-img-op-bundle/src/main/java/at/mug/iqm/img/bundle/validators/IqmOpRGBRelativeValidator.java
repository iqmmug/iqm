 ...
 * File: IqmOpRGBRelativeValidator.java
 * 
 * $Id$
 * $HeadURL$
 * 
...
public class IqmOpRGBRelativeValidator extends DefaultImageOperatorValidator {

	public IqmOpRGBRelativeValidator() {
	}
	...
	protected boolean validateSources(IWorkPackage wp)
		...
		// the sources must not be empty and must contain one non-single band image
			...
			if (box.getDataType() == DataType.IMAGE) {
				...
				if (ImageAnalyzer.has1Band(pi))
					throw new IllegalArgumentException(
							"This operator requires a non-single band image at index 0 of the source vector!");
			}
		}

 ...
 * File: FuzzyCMeansImageClustering.java
 * 
 * $Id$
 * $HeadURL$
 * 
 ...
 *  <li>2009 08 pixelArray[band] = (int)Math.round(clusterCenters[aCluster][band]); //Math.round added
 *  <li>2010 01 added progressbar support
 *  <li>2011 01 added 16 bit support
 *  <li>2014 02 changed progressbar writing (100% = maximal number of iteration)
 * </ul>
 * 
 * @author Helmut Ahammer
 * @since 2009 06 Original from Rafael Santos (rafael.santos@lac.inpe.br)
 * 
 */
...
public class FuzzyCMeansImageClustering extends ImageProcessingTask {
	 ...
	 */
	public FuzzyCMeansImageClustering(PlanarImage pInput, int numClusters,
	...
	public void run() {
		...
		for (iteration = 0; iteration < maxIterations; iteration++) {
			...
			int proz = (iteration + 1) * 100 / maxIterations;
			...
			if (Math.abs(lastJ - j) < epsilon) {
				BoardPanel.appendTextln("FuzzyCMeansImageClustering: Objective function smaller than eps");
				break;
			}
			lastJ = j;
		} // end of the iterations loop.
		hasFinished = true;
		BoardPanel.appendTextln("FuzzyCMeansImageClustering: Finished at iteration: " + iteration);
		...
		for (int c = 0; c < numClusters; c++)
			for (int h = 0; h < height; h++) {
				...
				for (int w = 0; w < width; w++) {
					...
					// Top is the distance of this data point to the cluster
					// being read.
		...
		for (int h = 0; h < height; h++)
			for (int w = 0; w < width; w++) {
				...
				for (int band = 0; band < numBands; band++)
					pixelArray[band] = Math.round(clusterCenters[aCluster][band]); // Math.round added 2009 08
		...
		for (int h = 0; h < height; h++)
			for (int w = 0; w < width; w++) {
				...
				for (int c = 0; c < numClusters; c++) {
					...
					cs += membership[w][h][c] * membership[w][h][c] * distancePixelToCluster * distancePixelToCluster;
				}
			}
		cs /= (height * width);
